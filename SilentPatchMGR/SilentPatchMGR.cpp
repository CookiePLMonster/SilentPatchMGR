...
static void InitASI()
{
	...
	if (int skipIntros = GetPrivateProfileIntW(L"SilentPatch", L"SkipIntroSplashes", -1, GetINIPath().c_str()); skipIntros != -1)
	{
		if (skipIntros != 0)
		{
			DWORD skipLogoSequences = base + 0x652C9A; // aob was broken
			Patch<uint8_t>(skipLogoSequences, 0xEB); // je -> jmp
		}
	}

	if (int fixedWigB = GetPrivateProfileIntW(L"SilentPatch", L"FixedWigB", -1, GetINIPath().c_str()); fixedWigB != -1)
	{
		if (fixedWigB != 0)
		{
			DWORD disableOnBladeMode = base + 0x7F1B41;
			DWORD enableOnBladeMode = base + 0x7C33B8;

			Patch<uint8_t>(disableOnBladeMode, 0xEB);
			Patch<uint8_t>(enableOnBladeMode, 0xEB);
		}
	}
